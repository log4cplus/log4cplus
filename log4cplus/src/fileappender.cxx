...
namespace log4cplus
{
	...
	namespace
	{
		...
		// The reason for the static declaration is 'ugly naming in DLLs'.

		static int
		file_rename (const tstring& src, const tstring & target)
		{
#if defined (UNICODE) && defined (WIN32)
			int returnValue = _wrename (src.c_str (), target.c_str ());
#else
			int returnValue = std::rename(LOG4CPLUS_TSTRING_TO_STRING (src).c_str(), 
		...
		{
#if defined (UNICODE) && defined (WIN32)
			int returnValue = _wremove (src.c_str ());
#else
			int returnValue = std::remove(LOG4CPLUS_TSTRING_TO_STRING(src).c_str());
#endif
			if (returnValue)
				returnValue = errno;
			return returnValue;
		...
		static void
		loglog_renaming_result (helpers::SharedObjectPtr<helpers::LogLog> const & loglog,
								const tstring& src, 
		...
		static
		void
		loglog_opening_result (const helpers::SharedObjectPtr<helpers::LogLog>& loglog,
							   const log4cplus::tostream& os, 
							   const tstring & filename)
		{
			if (! os)
			{
				loglog->error (LOG4CPLUS_TEXT("Failed to open file ") 
							   + filename);
			}
		}
		...
		rolloverFiles(const tstring& filename, unsigned int maxBackupIndex)
		{
			...
			for (int i = maxBackupIndex - 1; i >= 1; --i)
			{
				...
				const tstring source = source_oss.str();
				const tstring target = target_oss.str();
	...
	FileAppender::FileAppender(const Properties& _properties, 
							   LOG4CPLUS_OPEN_MODE_TYPE mode)
		: Appender(_properties), immediateFlush(true)
	{
		const tstring& filenameProperty = _properties.getProperty( LOG4CPLUS_TEXT("File") );
		...
		else
		{
			...
			if(_properties.exists( LOG4CPLUS_TEXT("ImmediateFlush") )) 
			{
				const tstring& tmp = helpers::toLower(_properties.getProperty(LOG4CPLUS_TEXT("ImmediateFlush")));
				immediateFlush = (tmp == LOG4CPLUS_TEXT("true"));
			}
			if(_properties.exists( LOG4CPLUS_TEXT("Append") )) 
			{
				const tstring tmp = helpers::toLower(_properties.getProperty(LOG4CPLUS_TEXT("Append")));
				mustAppend = (tmp == LOG4CPLUS_TEXT("true"));
			}
					   ...
					   LOG4CPLUS_OPEN_MODE_TYPE mode)
	{
		...
		{
			getErrorHandler()->error(  LOG4CPLUS_TEXT("Unable to open file: ") + _filename);
		}
	}
		...
		: FileAppender(properties, std::ios::app)
	{
		...
		if(properties.exists( LOG4CPLUS_TEXT("MaxBackupIndex") ))
		{
			...
			maxIndex = std::atoi(LOG4CPLUS_TSTRING_TO_STRING(tmp).c_str());
		}
		  ...
		  maxBackupIndex(10)
	{
		...
		if(properties.exists( LOG4CPLUS_TEXT("MaxBackupIndex") )) 
		{
			const tstring tmp = properties.getProperty(LOG4CPLUS_TEXT("MaxBackupIndex"));
	...
	void
	DailyRollingFileAppender::init(DailyRollingFileSchedule _schedule)
	{
