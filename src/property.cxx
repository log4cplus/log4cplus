...
namespace log4cplus { namespace helpers {
		...
		namespace
		{
			...
			{
				input.erase(input.begin(), std::find_if(input.begin(), input.end(), IsNotSpace()));
				return input;
			}
			...
			{
				return tstring(std::find_if(input.begin(), input.end(), IsNotSpace()), input.end());
			}
			...
			{
				input.erase(std::find_if(input.rbegin(), input.rend(), IsNotSpace()).base(), input.end());
				return input;
			}
			...
			{
				return tstring(input.begin(), std::find_if(input.rbegin(), input.rend(), IsNotSpace()).base());
			...
			if (input)
			{
				...
				while (std::getline (input, buffer))
				{
					...
					if (!buffer.empty() && (*buffer.begin() != PROPERTIES_COMMENT_CHAR) )
					{
						...
						if (*buffer.rend() == LOG4CPLUS_TEXT('\r')) // Remove trailing 'Windows' \r.
							buffer.resize (buffer.size() - 1);
						
						const tstring::size_type idx = buffer.find('=');
						if (idx != tstring::npos)
							setProperty(trim_right_copy(buffer.substr(0, idx)), trim_copy(buffer.substr(idx + 1)));
					}
				}
			}
		}
		...
		{
			Properties ret;
			for (map_type::const_iterator iter = data_map.begin(); iter != data_map.end(); ++iter)
			{
