...
namespace log4cplus
{
	...
	PropertyConfigurator::configureLoggers()
	{
		...
		for(std::set<tstring>::const_iterator it=loggers.begin(); it!=loggers.end(); ++it)
		{
		...
		else
		{
			...
			while (iter != iter_end)
			{
				if ( (appenderIt = appenders.find(*iter)) == appender_end)
				{
					...
					errorBuffer << LOG4CPLUS_TEXT("PropertyConfigurator::configureLogger()- Invalid appender: ") << *iter;
	...
	PropertyConfigurator::configureAppenders()
	{
		...
		Properties appenderProperties = properties.getPropertySubset(LOG4CPLUS_TEXT("appender."));
		...
		for(; props_iter != props_iter_end; ++props_iter)
		{
			try
			{
				...
				{
					AppenderFactory* factory = getAppenderFactoryRegistry().get(props_iter->second);
					if (!factory)
					{
						tostringstream errorBuffer;
						errorBuffer << LOG4CPLUS_TEXT("PropertyConfigurator::configureAppenders()")
					...
					else
					{
						...
						Properties tmpProperties = appenderProperties.getPropertySubset(props_iter->first + LOG4CPLUS_TEXT("."));
						SharedAppenderPtr appender = factory->createObject(tmpProperties);
						if (!appender)
						{
							tostringstream errorBuffer;
							errorBuffer << LOG4CPLUS_TEXT("PropertyConfigurator::configureAppenders()- Failed to create appender: ") 
										<< props_iter->first;
							getLogLog().error(errorBuffer.str());
						}
						else
	...
	PropertyConfigurator::configureAdditivity()
	{
		...
		while (iter != iter_end)
		{
			...
			const std::string& actualValue = additivityProperties.getProperty(*iter);
			const std::string& lowerValue = toLower(actualValue);
